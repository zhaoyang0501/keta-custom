...
<dwz:paginationForm action="${contextPath }/management/opinion/opinion/listOpinionSelf" page="${page }">
	<input type="hidden" name="search_LIKE_title" value="${param.search_LIKE_title}"/>
</dwz:paginationForm>

<form method="post" action="${contextPath }/management/opinion/opinion/listOpinionSelf" onsubmit="return navTabSearch(this)">
	...
	<table class="table" layoutH="162" width="100%">
		...
		<tbody>
			...
			<tr target="slt_uid" rel="${item.id}">
				...
				<c:choose>
					<c:when test="${item.opinionStatus=='708'}">
						<a href="${contextPath }/management/opinion/opinion/self?id=${item.id}" target="dialog" width="800" height="500" >派遣</a>
					</c:when>
					<c:when test="${item.opinionStatus=='707'}">
						<a href="${contextPath }/management/pubOpinion/pubOpinion/accept?id=${item.pubopinion.id}" target="dialog" width="800" height="500" >已退回</a>
					</c:when>
					<c:when test="${item.opinionStatus=='712'}">
						<a href="${contextPath }/management/opinion/opinion/aphcOpinion?id=${item.id}" target="dialog" width="800" height="500" >安排检查</a>
					</c:when>
				</c:choose>
				</td>
			</tr>

...
<dwz:paginationForm action="${contextPath }/management/opinion/opinion/listOpinionCheck2" page="${page }">
	<input type="hidden" name="search_LIKE_title" value="${param.search_LIKE_title}"/>
</dwz:paginationForm>

<form method="post" action="${contextPath }/management/opinion/opinion/listOpinionCheck2" onsubmit="return navTabSearch(this)">
		...
		<tbody>
			...
			<tr target="slt_uid" rel="${item.id}">
				...
				<td>
					...
					<a href="${contextPath }/management/opinion/opinion/check2?id=${item.id}" target="dialog" width="800" height="500" >处理反馈</a>
					<a href="${contextPath }/management/opinion/opinion/check2back?id=${item.id}" target="dialog" width="800" height="500" >回退</a>
				</td>
			</tr>
			</c:forEach>
		</tbody>
